...
pacman::p_load(dplyr, tidyr, purrr, rlang, forcats, ggplot2, scales, ggpubr, here)

#### Crear Outcomes ####

df <- df %>%
  mutate(
    anemia = if (!"anemia" %in% names(df)) if_else(as.numeric(P037) < 10.5, 1L, 0L, missing = NA_integer_) else anemia,
    ferropenia = if (!"ferropenia" %in% names(df)) if_else(as.numeric(ferr_adj) < 12, 1L, 0L, missing = NA_integer_) else ferropenia,
    anemia_no_ferropenica = if (!"anemia_no_ferropenica" %in% names(df)) if_else(anemia == 1L & ferropenia == 0L, 1L, 0L, missing = NA_integer_) else anemia_no_ferropenica,
    anemia_ferropenica    = if (!"anemia_ferropenica"    %in% names(df)) if_else(anemia == 1L & ferropenia == 1L, 1L, 0L, missing = NA_integer_) else anemia_ferropenica
  )

#### Modelos de Regresión ####

fit_logbinomial_safe <- function(formula, data) {
  m_pois <- glm(formula, family = poisson(link = "log"), data = data)
  m_bin  <- try(glm(formula, family = binomial(link = "log"), data = data, start = coef(m_pois)),
                silent = TRUE)
  if (inherits(m_bin, "try-error")) m_pois else m_bin
}
...
plot_margins_logbin <- function(df, outcome, letter,
                                x_var,
                                group_var = "etnia",
                                covars,
                                ymax,
...
pA_edu <- plot_margins_logbin(df, "anemia", "A",
                              x_var = "educacion",
                              covars = c("sexo", "wealth_3", "DEP"),
                              ymax = 0.75,
                              x_lab = "\nNivel de Educación")

pB_edu <- plot_margins_logbin(df, "ferropenia", "B",
                              x_var = "educacion",
                              covars = c("sexo", "wealth_3", "DEP"),
                              ymax = 0.50,
                              x_lab = "\nNivel de Educación")

pC_edu <- plot_margins_logbin(df, "anemia_no_ferropenica", "C",
                              x_var = "educacion",
                              covars = c("sexo", "wealth_3", "DEP"),
                              ymax = 0.60,
                              x_lab = "\nNivel de Educación")

pD_edu <- plot_margins_logbin(df, "anemia_ferropenica", "D",
                              x_var = "educacion",
                              covars = c("sexo", "wealth_3", "DEP"),
                              ymax = 0.50,
                              x_lab = "\nNivel de Educación")

Fig_Edu_ABCD <- ggpubr::ggarrange(pA_edu, pB_edu, pC_edu, pD_edu,
                                  ncol = 2, nrow = 2,
                                  common.legend = TRUE, legend = "bottom")

ggsave(here::here("outputs","Figure_Education.png"),
       Fig_Edu_ABCD, width = 10.73, height = 10.23, dpi = 1200)

#### Generación de Figuras para Riqueza ####

pA_w <- plot_margins_logbin(df, "anemia", "A",
                            x_var = "wealth_3",
                            covars = c("sexo", "educacion", "DEP"),
                            ymax = 0.75,
                            x_lab = "\nÍndice de Riqueza")

pB_w <- plot_margins_logbin(df, "ferropenia", "B",
                            x_var = "wealth_3",
                            covars = c("sexo", "educacion", "DEP"),
                            ymax = 0.50,
                            x_lab = "\nÍndice de Riqueza")

pC_w <- plot_margins_logbin(df, "anemia_no_ferropenica", "C",
                            x_var = "wealth_3",
                            covars = c("sexo", "educacion", "DEP"),
                            ymax = 0.50,
                            x_lab = "\nÍndice de Riqueza")

pD_w <- plot_margins_logbin(df, "anemia_ferropenica", "D",
                            x_var = "wealth_3",
                            covars = c("sexo", "educacion", "DEP"),
                            ymax = 0.50,
                            x_lab = "\nÍndice de Riqueza")

Fig_Wealth_ABCD <- ggpubr::ggarrange(pA_w, pB_w, pC_w, pD_w,
                                     ncol = 2, nrow = 2,
                                     common.legend = TRUE, legend = "bottom")

ggsave(here::here("outputs","Figure_Wealth.png"),
       Fig_Wealth_ABCD, width = 10.73, height = 10.23, dpi = 1200)